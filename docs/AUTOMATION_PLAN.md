...
| **Anchor esleyici + aligner entegrasyonu** ✅ v1: MusicXML YAZILI (tekrar acilmamis) event yapisi + satir-sirali kalibrasyon (imza kumesi k-eleme, 12pt yinelemeli aykiri eleme, satirlar arasi beat ekstrapolasyonu). Aligner `output/.../note-anchors.generated.json` varsa OTOMATIK kullanir ve giris bazinda en iyi hizalamayi secer (`pickBetterAlignment`: anchor yalniz medyan deltayi dusurup coverage'i ~%5'ten fazla kaybetmiyorsa kazanir — regresyon garantisi). Sonuc: **89 high** (baseline 85, +4), import dry-run 1.716 kutuyla gecti, repair-proposal 13.650 replace/740 keep. **Kok bulgu:** TXT tekrarlari actigi icin tekrarli eserlerde anchor/event orani ~1/tekrar katsayisi (641 giris count-mismatch OLÇÜLDÜ) | written<->expanded olcu eslemesi (641 repeat giris) siradaki P1; kalibrasyon iyilestirmesi (satir chunking) P2 | P1 — ENTEGRASYON TAMAM |
