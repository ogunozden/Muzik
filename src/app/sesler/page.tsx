...
export default function SeslerPage() {
  ...
  const testActiveGroup = useCallback(async () => {
    ...
    if (activeGroupInstalledCount === 0) {
      setMessage(`${activeGroup || "Bu enstrüman"} için sample yüklü değil. Önce bu grupta bir slot yükle.`);
      return;
    }
        ...
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className={`mt-2 max-w-3xl text-sm ${tokens.colors.text.secondary}`}>
              Her slot sabit bir dosya adına yazılır. Dosyayı değiştirdiğinde çalma motoru yeni sample&apos;ı kullanır. Sample yüklü değilse synth taklit çalmaz.
            </p>
          </div>
