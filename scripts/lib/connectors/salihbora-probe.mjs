...
export const CONNECTOR_ID = "salihbora";
...
export async function searchSalihBora(query, timeoutMs) {
  const browser = await chromium.launch({ headless: true });
  try {
    const page = await browser.newPage();
    ...
    while ((m = regex.exec(html)) !== null) {
      if (m[1].includes('salihbora.com')) links.push(m[1]);
    }
    if (links.length === 0) {
...
export async function verifySalihBoraGroup({group, provider, checkedAt, timeoutMs=8000, rows=3, cache, acceptedThreshold=80}) {
  ...
  const links = await searchSalihBora(query, timeoutMs);
  ...
  for (const url of links.slice(0, rows)) {
    ...
    scored.push({
      url,
      title: probe.htmlTitle,
      bodyText: probe.bodyText.slice(0, 200),
      confidence,
    });
  }
  ...
  const result = {
    ...
    checkedAt,
    searchQuery: query,
    searchUrl: `site:salihbora.com ${group.title} ${group.composer}`,
