...
import {
  ...
  DEDUPE_REPORT_VERSION,
  buildCoverageMatrix,
  buildDedupeReport as buildDedupeReportBase,
  summarizeCounts,
...
export function buildDedupeReport({
  ...
  return buildDedupeReportBase({
    bulkCandidates,
    acceptedBulkCandidates,
    candidateReviewRows,
    generatedAt,
    getAcceptedIdentity: getReferenceIdentity,
  });
}
...
export function runExternalReferenceCoverageAudit({
  ...
  const candidateReviewBatchPlanJsonPath = path.join(
    safeOutDir,
  ...
  const candidateReviewBatchPlan = buildCandidateReviewBatchPlan(candidateReviewGroups, candidateReviewRows, {
    generatedAt,
