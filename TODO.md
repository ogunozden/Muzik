...
| W4.1c ✅ | **Nota-anchor extractor + anchor eşleyici v1**: PDF metin katmanından nota başı glifleri (TT7/F3 müzik fontları), font-genişlik bilinçli konum hesabı (Tc/Tw/Tz/kerning), MusicXML yazılı (tekrarsız) event yapısıyla satır-sıralı kalibrasyon + aykırı eleme. Korpus: **1.157 kalibre** giriş (4.727 satır artığı ≤3pt), 641 repeat-etkili ölçüldü, 990 staff-çizgisiz (tarama PDF), 204 zip-inflate (mevcut extractor ile aynı küme) | `extract:symbtr-note-anchors` → `output/symbtr-layout-review/note-anchors.generated.json` | KAPANDI (2026-08-08) — aligner entegrasyonu + written↔expanded ölçü eşlemesi P1 (AUTOMATION_PLAN §1) |
