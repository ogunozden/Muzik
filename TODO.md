...
| W3.1 | GitNexus FTS indeksleri eksik (grafik indeksi tazelendi; FTS uzantısı bu makinede yüklenemiyor) | 5 deneme + `gitnexus doctor` (2026-08-08): `--repair-fts` → "LOAD fts failed after successful INSTALL"; doctor FTS'i "available" gösterse de analyze süreci `load-only/offline` politikayla uzantıyı Node SQLite'a yükleyemiyor; VECTOR bu platformda kapalı (semantic = exact-scan) | P2 · ÇEVRESEL (kod tarafı yok) — Node/GitNexus sürümü veya makine politikası değişince `node .gitnexus/run.cjs analyze --repair-fts` tekrar dene |
...
| W4.3b ✅ | **Oto-yeniden-tarama kancası**: sample havuzu manifest sha256'sı değişince claimed klasörler otomatik yeniden korelasyonla taranır (`samples:auto-rescan-claimed`); idempotent. İlk koşu 4 klasörü yeniden ölçtü (lavta .75 / santur .60 / rebab .81 / kasık .18 — eşik altı, önceki ölçümle tutarlı); ikinci koşu 0,7s'de "havuz değişmedi" | `output/samples/claimed-pool-fingerprint.json` | KAPANDI (2026-08-08) |
| W4.2a ✅ | Zamanlanmış koşucu + deterministik kabul eşiği uygulandı: cache-bazlı kaldığı-yer, throttle, offline/stall algılama, kümülatif accepted manifest; otomatik-doğrulanabilir backlog **2.973/2.973 (%100)**; 18 accepted-ready import dry-run GEÇTİ (19 duplicate-URL elendi); kalan 5 conflict/deferred insan kürasyonu | kök düzeltmeler: offset bug, ogm ReferenceError, boş-sorgu null URL, maxResponseBytes 768KB, TDZ, manifest kaybı | KAPANDI (2026-08-08) — insan yüzeyi: 5 grup |
| W4.2b ✅ | **Duplicate-URL demotion**: `scripts/demote-duplicate-url-candidates.mjs` — accepted-ready cache satırları URL kimliğine göre deterministik ayrıştırıldı (37 → 18 kazanan); 19 kaybeden `conflict`'e demote edildi (neden + kazanan kayıtlı, `demotedAt`'li); `verify:external-source-providers:schedule` AYNI kaldı: 0 parti, 0 kalan, 18 accepted, duplicateUrlExcludedCount 0 | demote listesi: `output/external-source-discovery/duplicate-url-demote-list.json`; cache `--write` ile güncellendi; koşucu conflict'e dokunmaz (AUTO_VERIFIABLE_STATUSES = needs-review) | KAPANDI (2026-08-08) — insan yüzeyi yalnız 5 conflict/deferred grup |
...
| W4.1d ✅ | **Written↔expanded ölçü eşlemesi + motor sözleşmesi**: `expandWrittenMeasures` (repeat+volta+segno/D.S.); aligner written-expanded yolunda kutular **ilk-genişlemiş (1-bazlı) ölçü indeksini** taşır. Motor sözleşmesi genişletildi: `RUNTIME_ACCEPTED_MEASURE_INDEX_BASES` (TS `layout.ts` + mjs, eşitlik testli), import kapısı + manifest doğrulayıcı written-expanded-v1'i mapping zarfıyla kabul eder, temizlik script'i yeni basis'i silmez. Import-ready **104 giriş** (89+16) dry-run **GEÇTİ** (1.981 kutu, 0 hata); rapor **104 high / 860 medium / 1.831 low**, storedMismatch 14.370 | Manifeste YAZMA hâlâ operatör onayı bekliyor; karmaşık volta/yuvalanmış açılım (korpus %56) P2 | KAPANDI (2026-08-08) — yazım onayı P1 |
| W4.1e ✅ | **Walk-rehberli D.S. açılımı**: TXT walk ölçü sayısı hedef alınarak D.S. bölümünün bitişi deterministik çözülüyor (SymbTr TXT'lerinde `<fine>` çoğu zaman işaretli değil; segno→son varsayımı 1.215 vakayı fazla uzatıyordu). Korpus birebir eşleşme **%43,8 → %59,5** (+469 giriş). Rapor: **110 high / 886 medium / 1.799 low** (baseline 85/770/1.940), storedMismatch 13.925 (−769), written-expanded 365 giriş; import-ready **110 giriş** dry-run geçti (2.159 kutu, 0 hata) | manifest yazımı operatör onayı bekliyor | KAPANDI (2026-08-08) |
| W4.1f ✅ | **Yapısal yorum araması (`expandWrittenMeasuresGuided`)**: repeat+DS etkileşiminde aday yorumlar (segno konumları × önce/sonra segment tekrarlı/ham × DS-sonu walk-rehberli) üretilir, TXT walk ile birebir eşleşen seçilir. Baskın çözüm: **D.S. bölümü iç tekrarları olmadan çalınır** (`after-raw:ds-guided`). Korpus birebir eşleşme **%59,5 → %75,9** (2.268 giriş, +490). Rapor: **116 high / 928 medium / 1.751 low** (baseline 85/770/1.940 → +31/+158/−189), storedMismatch 13.391 (−1.303), written-expanded 502 giriş; import-ready **116 giriş** dry-run geçti (2.275 kutu, 0 hata) | manifest yazımı operatör onayı bekliyor | KAPANDI (2026-08-08) |
| W4.1g ✅ | **Dalsegno-ölçüsü yorumu (before-exclusive)**: D.S. atlamasında dalsegno ölçüsünün öncesinde çalınıp çalınmadığı da aday olarak ölçülür — 24 giriş daha birebir (korpus **%76,7**). Rapor: **116 high / 932 medium / 1.747 low**, written-expanded 510 giriş; import-ready 116 giriş dry-run geçti | — | KAPANDI (2026-08-08) |
| W4.1 ✅ | **Manifeste YAZMA — KAPANDI**: 116 import-ready giriş (84 meter-walk-v2 + 32 written-expanded-v1) yazıldı → manifest **624 giriş / 20.594 kutu** (592 + 32 basis; 32'sinde writtenMeasureMapping); `verify:symbtr-layout-review-import` + `verify:symbtr-measures` errors: []; layout testleri yeni sayımlara güncellendi (1107/1107 yeşil); tüm artefaktlar (aligner, import-ready, review template, repair-proposals) tazelendi | import kapısı kanıt zarfı + test güncellemeleri commit (2026-08-08) | KAPANDI (2026-08-08) — kalan: 1.181 aday + korpus %23,3 volta/nested = insan/müzikal karar yüzeyi |
| W4.1h ✅ | **Repair-proposals UYGULAMASI — operatör onaylı**: `apply:symbtr-repair --write` (yeni `scripts/apply-symbtr-repair-proposals.mjs`, 2 testli). Kapsam birebir: **12.008 replace** uygulandı (önerilen aday geometrisi, reviewer symbtr-txt-aligner-v1), 2.864 keep korundu, **3.607 çakışan review kutusu** verified'dan düştü (aday kümesinde kaldı; aynı PDF adayına iki ölçü işaret edemez kuralı), 459 add YAZILMADI (ayrı import-ready kapısı). Preview son otorite doğrulayıcıdan 0 hata ile geçti. Sonuç: manifest **624 giriş / 16.987 kutu** (503 giriş repairEvidence taşır); storedMismatch **12.863 → 285 (−%97,8)**; taze proposal replace **0** / review 2.115 / add 1.029; kapılar: verify errors [], 1111 test, prod-cycle OK | repair-apply raporu: `output/symbtr-layout-review/repair-apply-report.json` | KAPANDI (2026-08-08) — kalan insan/görsel yüzey: 2.115 review kutusu + 1.029 add adayı |
...
| W4.1 | Verified PDF ölçü kutusu: 624 giriş / 16.987 kutu verified; 1.181 giriş hâlâ vektör adayı + 2.115 review kutusu + 1.029 add adayı | kalan adaylar insan/görsel onay gerektirir (`UX_UI_COMPLETION_AUDIT.md`) | Dış girdi |
