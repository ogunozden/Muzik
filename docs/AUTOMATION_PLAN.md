...
| **Written↔expanded olcu eslemesi** ✅ (rapor duzeyi): `expandWrittenMeasures` — `<repeat>`+volta VE `<segno>`+D.S. navigasyonunu acar; TXT walk ile BIREBIR eslesenler (korpus 43,8%; navigasyonlu dosyalarin 651'i) written-expanded analiz yoluna girer: kutular ilk-genislemis olcu indeksini tasir (runtime expanded uzayinda esler — follow UI). Sonuc: **104 high / 860 medium / 1.831 low** (baseline 85/770/1.940), storedMismatch 14.370 (−324), 241 tekrar giris analiz edildi (contained medyan 0.94) | MOTOR SOZLESMESI DEGISIMI: yeni basis (`written-expanded-v1`) icin TS `layout.ts` + `symbtr-score-measures` + import kapisi genisletmesi + manifest yazimi — operatör onayi; karmaşık volta/yuvalanmış acilim (korpus %56) P2 | P1 — ANALIZ TAMAM, YAZIM ONAY BEKLIYOR |
